...
namespace junebug
{
    ...
    void DrawPolygonOutline(const std::vector<Vec2<double>> &vertices, const Color &color, const Vec2<float> offset, const float thickness = 1.0f);
    inline void DrawPolygonOutline(const std::vector<Vec2<double>> &vertices, const Color &color, const float thickness = 1.0f) { DrawPolygonOutline(vertices, color, Vec2<float>::Zero, thickness); }
